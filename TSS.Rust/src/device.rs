...
impl TpmDevice for TpmTcpDevice {
    fn connect(&mut self) -> Result<bool, TpmError> {
        ...
        command_socket.set_read_timeout(Some(Duration::from_secs(5)))
            .map_err(|e| TpmError::IoError(e.to_string()))?;
        signal_socket.set_read_timeout(Some(Duration::from_secs(5)))
            .map_err(|e| TpmError::IoError(e.to_string()))?;
    ...
    fn dispatch_command(&mut self, cmd_buf: &[u8]) -> Result<(), TpmError> {
        if let Some(mut socket) = self.command_socket.as_ref().map(|s| s.try_clone().unwrap()) {
            ...
            socket.write_all(&[self.locality])
                .map_err(|e| TpmError::IoError(e.to_string()))?;
            Self::send_int(&mut socket, cmd_buf.len() as u32)?;
            
            // Send the command data
            socket.write_all(cmd_buf)
                .map_err(|e| TpmError::IoError(e.to_string()))?;
    ...
    fn response_is_ready(&self) -> Result<bool, TpmError> {
        if let Some(socket) = &self.command_socket {
            ...
            let mut read_set = [socket.try_clone()
                .map_err(|e| TpmError::IoError(e.to_string()))?];
            
            // Check if there's data to read with a zero timeout
            match socket::select(&mut read_set, &mut [], &mut [], Some(&Duration::from_secs(0))) {
                Ok(1) => Ok(true),
                Ok(_) => Ok(false),
...
impl TpmDevice for TpmTbsDevice {
    fn connect(&mut self) -> Result<bool, TpmError> {
        ...
        use windows::Win32::Security::Tbs::*;
        
        if !self.context.is_null() {
            return Ok(true); // Already connected
        }
        
        let mut parms = TBS_CONTEXT_PARAMS2 {
        ...
        let res = unsafe {
            Tbsi_Context_Create(
                &parms as *const TBS_CONTEXT_PARAMS2 as *const TBS_CONTEXT_PARAMS,
                context_ptr
            )
        };
        
        if res != TBS_SUCCESS {
            return Err(TpmError::TbsError(format!("Failed to connect to TBS: {:?}", res)));
        }
        
        // Get device info to check if TPM 2.0 is available
        let mut info = TPM_DEVICE_INFO::default();
        let res = unsafe {
            Tbsi_GetDeviceInfo(std::mem::size_of::<TPM_DEVICE_INFO>() as u32, &mut info)
        };
    ...
    fn dispatch_command(&mut self, cmd_buf: &[u8]) -> Result<(), TpmError> {
        ...
        if res != TBS_SUCCESS {
            return Err(TpmError::TbsError(format!("TBS SubmitCommand error: 0x{:08x}", res)));
        }
    ...
    fn connect(&mut self) -> Result<bool, TpmError> {
        ...
        match OpenOptions::new().read(true).write(true).open("/dev/tpm0") {
            ...
            Err(_) => {
                ...
                match OpenOptions::new().read(true).write(true).open("/dev/tpmrm0") {
                    Ok(file) => {
                        self.dev_tpm = Some(file);
                        self.tpm_info = TpmConnInfo::TpmTbsConn as u32 | 
                                       TpmConnInfo::TpmUsesTrm as u32 |
        ...
        if self.tpm_info & (TpmConnInfo::TpmSocketConn as u32) != 0 {
            ...
            if let Some(dev) = self.dev_tpm.as_mut() {
                ...
                match dev.read(&mut resp_buf) {
                    Ok(bytes_read) => {
                        if bytes_read < 10 { // 10 is the mandatory response header size
                            return Err(TpmError::IoError(format!(
                                "Failed to read sufficient data from TPM: got {} bytes", bytes_read
                            )));
                        }
...
mod socket {
    ...
    pub fn select(
        ...
        use std::os::unix::io::AsRawFd;
        use libc::{fd_set, timeval, select, FD_SET, FD_ZERO, FD_ISSET};
    ...
    pub fn select(
        ...
        use windows::Win32::Networking::WinSock::{fd_set, timeval, select, FD_SET, FD_ZERO, FD_ISSET};
